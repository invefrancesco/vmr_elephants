...
log.lik.VM <- function(par, data, formula, response) {
  # Complete cases
  data <- data %>%
    dplyr::select(all.vars(formula), response) %>%
    tidyr::drop_na()
...
acf_circular <- function(lag, x) {
  res_lag <- x %>% # nolint: object_usage_linter.
    dplyr::mutate(t2_lag = t2_ + lubridate::hours(lag * 4))

  res_lag <- res_lag %>% # nolint: object_usage_linter.
    dplyr::inner_join(
      residuals,
      by = c("t2_lag" = "t2_")
    )
