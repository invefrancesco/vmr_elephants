...
```
# Analisi del primo elefante (id: data)
...
```{r visualizzazione}
# data
data <- read_csv(paste0(dir_data, "/GON09.csv"))
# turn angle
ggplot(data = data, aes(x = ta_, y = after_stat(density))) +
...
tibble(
  parameter = factor(
    c(
      "Intercept", "Distance from water", "Elevation", "NDVI Index",
...
expand_grid(
  ...
  mutate(
    ...
    mu =
      2 * atan(fit_std$par[1] +
                 distriv_seq * fit_std$par[2] +
                 seasCD * fit_std$par[5] +
                 seasHD * fit_std$par[6])
  ) %>%
...
residuals <- data %>%
  ...
  mutate(
    ...
    ta_hat = 2 * atan(fit_std$par[1] +
                        fit_std$par[2] * data$distriv_std +
                        fit_std$par[3] * data$elev_std +
                        fit_std$par[4] * data$ndvi_std +
  ...
  mutate(
    res = (ta_ - ta_hat),
    time = round_date(t2_, unit = "hours")
  ) %>%
  select(time, res)

# Circular Autocorrelation Function -------------------------------------------
acf_circular <- function(lag, x, time) {
  xlag <- x %>% # nolint
    dplyr::mutate(timelag = time + lubridate::hours(lag * 4))
  ...
  cor <- circular::cor.circular(xlag$res.x, xlag$res.y, test = TRUE)
  tibble(
    lag = lag,
    n_match = n_match,
    acf = cor[[1]],
...
acf <- map_dfr(
  .x = 1:42,
  .f = ~ acf_circular(lag = .x, x = residuals, time = t2_)
)
# risultati ----
acf %>%
  mutate(
...
La funzione di autocorrelazione mostra una correlazione significativa, cioè i residui al tempo $t-1$ (4 ore) sono correlati con i residui al tempo $t$, e non è quindi possibile assumere indipendenza tra le osservazioni, la funzione di log-verosimiglianza è ridefinita per tenerne conto come segue: 
...
```{r AR1}
# Fit (La funzione si trova nel file _functions.R)
formula <- ta_ ~ sl_ + distriv_std + ndvi_std + elev_std + seas

fit_ar1 <- vector("list", 5)
for (i in 1:5) {
  tryCatch(
    fit_ar1[[i]] <- optim(
      par = c(rnorm(8),
...
# par <- fit_ar1$par
# x <- model.matrix(
#   ~ sl_ + distriv_std + ndvi_std + elev_std + seas,
#   data
# )
# y <- data$ta_
# burst <- data$burst_
...
#   eta = as.vector(eta),
#   sl_ = data$sl_,
#   distriv_std = data$distriv_std,
#   ndvi_std = data$ndvi_std,
#   elev_std = data$elev_std,
#   seas = data$seas
# ) %>%
#   group_by(burst) %>%
#   mutate(
...
# 
# # PLOT COVARIATE
# ggplot(data = l_data, aes(x = l_data$distriv_std, y = l_data$res)) +
#   geom_point() +
#   theme_test() +
#   geom_hline(
#     yintercept = 0,
...
#   )
# 
# ggplot(data = l_data, aes(x = l_data$sl_, y = l_data$res)) +
#   geom_point() +
#   theme_test() +
#   geom_hline(
#     yintercept = 0,
...
#   )
# 
# ggplot(data = l_data, aes(x = l_data$ndvi_std, y = l_data$res)) +
#   geom_point() +
#   theme_test() +
#   geom_hline(
#     yintercept = 0,
...
#   )
# 
# ggplot(data = l_data, aes(x = l_data$distriv_std, y = l_data$y)) +
#   geom_point(size = 0.1) +
#   theme_test() +
#   geom_hline(
#     yintercept = 0,
