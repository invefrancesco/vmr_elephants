...
l_data <- tibble(
  ...
  eta = as.vector(eta),
  sl_ = GON09$sl_,
  distriv_std  = GON09$distriv_std,
  ndvi_std = GON09$ndvi_std,
  elev_std = GON09$elev_std,
  seas = GON09$seas
) %>% 
  group_by(burst) %>% 
  mutate(
...
Ai fini del confronto delle performance dei due modelli, si specifica anche un modello autoregressivo più semplice della forma: 
...
```{r fit-AR1-alternative}
# fit (multistart) ----
fit_ar1_alt <- vector("list", 20)
for(i in 1:20) {
  tryCatch(
    fit_ar1_alt[[i]] <- optim(
      par = c(rnorm(8), log(2)),
      fn =  log.lik.VM.ar.alt,
      data = GON09,
      formula = ~ sl_ + distriv_std + ndvi_std + elev_std + seas,
      response = "ta_",
      burst = "burst_",
...
unlist(sapply(fit_ar1_alt, "[[", "value"))
fit_ar1_alt <- fit_ar1_alt[[which.min(unlist(sapply(fit_ar1_alt, "[[", "value")))]]
...
```{r Verifiche sul modello alternativo}
par <- fit_ar1_alt$par
X <- model.matrix(~ sl_ + distriv_std + ndvi_std + elev_std + seas,
                  GON09)
y <- GON09$ta_
...
# Residuals vs Fitted 
ggplot(l_data, aes(x = mu_t, y = res)) +
  geom_point() +
  theme_test() +
  geom_hline(yintercept=0, 
             linetype="dashed", 
