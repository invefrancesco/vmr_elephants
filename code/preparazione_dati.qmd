...
dat_all <- dat_all_tmp %>% 
  left_join(
    dplyr::select(data_elephants, id, XUTM, YUTM, distriv, ndvi, elev, seas),
    join_by("id" == "id", "x2_" == "XUTM", "y2_" == "YUTM")
  ) %>% 
  mutate("distriv_std" = scale(dat_all$distriv)[,1],
         "elev_std" = scale(dat_all$elev)[,1],
         "ndvi_std" = scale(dat_all$ndvi)[,1])
