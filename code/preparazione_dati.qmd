...
# covariate standardizzate
GON10_fnl <- GON10_fnl %>% 
  mutate("distriv_std" = scale(GON10_fnl$distriv)[,1],
...
# covariate standardizzate
GON34_fnl <- GON34_fnl %>% 
  mutate("distriv_std" = scale(GON34_fnl$distriv)[,1],
...
dat_all_tmp <- dat_all %>%
  ...
  mutate(stps = lapply(trk, function(x) { x %>% 
...
dat_all <- dat_all %>% 
  mutate("distriv_std" = scale(distriv)[,1],
