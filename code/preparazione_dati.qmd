...
data <- data_elephants %>% nest(data = -id)
...
data <- data %>%
  ...
  mutate(stps = lapply(trk, function(x) {
    x %>%
      ...
      amt::filter_min_n_burst(12) %>% # a burst of 12 steps means the elephant is followed for at least two days
      amt::steps_by_burst()
  })) %>%
  select(id, stps) %>%
  unnest()
...
save(data, file = paste0(dir_data, "/elephants.RData"))
```
