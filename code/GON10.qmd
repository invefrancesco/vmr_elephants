...
```{r visualizzazione}
# data 
GON10 <- read_csv(paste0(dir_data, "/GON10.csv"))
# turn angle
ggplot(data = GON10, aes(x = ta_, y = ..density..)) +
...
residuals <- GON10 %>%
  ...
  mutate(
    ...
    ta_hat = 2 * atan(fit_std$par[1] + 
                        fit_std$par[2] * GON10$distriv_std + 
                        fit_std$par[3] *  GON10$elev_std+ 
                        fit_std$par[4] * GON10$ndvi_std+ 
